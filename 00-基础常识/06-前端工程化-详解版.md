...
> 💡 **要点**：多进程编译（thread-loader）、外部扩展（externals）、DllPlugin 预编译、缓存策略、缩小文件搜索范围（include）。
...
> 💡 **要点**：CommonsChunkPlugin 提取公共代码、externals 分离第三方库、DllPlugin 预编译、Happypack 多线程加速。
...
> 💡 **要点**：基于 ESM 的开发服务器，esbuild 预构建依赖实现秒级启动，生产构建使用 Rollup。热更新仅编译变更模块，大型项目中大幅优于 Webpack。
...
> 💡 **要点**：Vercel 开发的 Rust 增量打包器，函数级缓存仅重新计算变更部分。Next.js 13+ 开发环境默认打包器，生态目前不如 Vite 成熟。
