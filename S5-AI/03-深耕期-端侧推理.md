...
**问题：** 生成每个新词都要重新计算所有历史词的Attention。生成N个词，计算量O(N³/3)。有KV Cache后降为O(N²/2)，N=4096时效率提升约2000倍。
...
```
M_KV = 2 × L × H_kv × S × P × B
L=层数, H_kv=KV头数, S=序列长度, P=精度(FP16=2), B=Batch Size
```

**实际对比：** Llama-70B用MHA时KV Cache=83.9GB，用GQA（8 KV heads）后降至10.5GB。这就是GQA的核心价值——大幅减少KV Cache。
...
| [⬅️ 上一阶段：进阶期](./02-进阶期-RAG应用.md) | [🏠 学习指南总览](./README.md#-ai-前端开发体系化学习指南) | [➡️ 下一阶段：专家期 - Agent](./04-专家期-Agent设计.md) |
|:---:|:---:|:---:|
