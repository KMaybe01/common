...
| **可观测** | 全链路可追踪、可监控 | OpenTelemetry、[LangSmith](https://smith.langchain.com)、自定义指标 |
...
| **流式输出** | SSE/[WebSocket](https://websockets.spec.whatwg.org) 流式响应 | 首字延迟 < 500ms | 低 |
...
| **执行层** | 沙箱隔离执行 | [Docker](https://docker.com) 容器隔离 |
| **输出层** | 内容过滤 | 敏感词库 + 分类模型 |
| **输出层** | 幻觉检测 | 事实核查 + 引用验证 |
| **监控层** | 审计日志 | 全链路日志记录 |
| **监控层** | 异常检测 | 行为模式分析 |
...
| 工具 | 用途 | 使用场景 |
|------|------|---------|
| **[LangSmith](https://smith.langchain.com)** | 全链路追踪 | 查看完整执行轨迹 |
| **OpenTelemetry** | 分布式追踪 | 微服务架构问题定位 |
| **Prometheus + [Grafana](https://grafana.com)** | 指标监控 | 性能指标可视化 |
...
| **故障恢复时间** | < 30s | 自动故障转移 |
| **数据一致性** | 强一致 | [Redis](https://redis.io) 集群 + 向量数据库 |
...
| 数据类型 | 存储介质 | 访问延迟 | 成本 |
|---------|---------|---------|------|
| 会话状态 | [Redis](https://redis.io) | < 1ms | 中 |
| 向量索引 | [Milvus](https://milvus.io)/[Pinecone](https://www.pinecone.io) | 5-20ms | 高 |
...
| **乐观锁** | 版本号检查 | 数据更新 |
| **分布式锁** | [Redis](https://redis.io) SETNX | 并发控制 |
| **状态机** | 状态流转检查 | 任务执行 |
...
| 集成测试 | [Docker](https://docker.com) Compose | 5 分钟 |
...
| **L3 可审计** | 不知道 Agent 做了什么 | 完整 Trace + 决策日志 + 回放 | [LangSmith](https://smith.langchain.com) / LangFuse |
...
| [⬅️ 上一部分：框架与工具链篇](./13-Agent面试题-框架工具链篇.md) | [🏠 返回主指南](./README.md) | [➡️ 下一部分：前沿趋势篇](./15-Agent面试题-前沿趋势篇.md) |
|:---:|:---:|:---:|
