...
| **资源管理** | ✅ 资源 + 订阅机制 | ❌ 仅函数调用 |
| **传输协议** | STDIO / SSE / [WebSocket](https://websockets.spec.whatwg.org) | HTTP (仅 REST) |
...
| 工具 | 原理 | 输出 | 适用阶段 |
|:---|:---|:---|:---|
| **v0.dev ([Vercel](https://vercel.com))** | LLM + Tailwind 模板 | [React](https://react.dev) + Tailwind 代码 | 原型设计 |
| **Bolt.new (StackBlitz)** | LLM + 浏览器 WebContainer | 完整项目代码 | 快速原型 |
| **Claude Artifacts** | Claude 直接渲染 | [React](https://react.dev)/SVG/HTML | 交互式预览 |
| **GPT-4o Canvas** | 多模态理解 + 生成 | 预览 + 代码 | 迭代编辑 |
| **Builder.io** | 设计稿 → 代码 | Figma → [React](https://react.dev)/Vue | 设计交付 |
...
### ⚡ [WebAssembly](https://webassembly.org).org) 在 AI 中的未来
...
| **WASI (WebAssembly(https://webassembly.org).org) System Interface)** | 系统级 API | — | 标准运行时接口 |
...
| **[Cursor](https://cursor.com)** | 整文件编辑 + Agent 模式 | $20/月 | ❌ | ⭐⭐⭐⭐⭐ |
| **Windsurf (Codeium)** | Flow 模式 + 深度上下文 | 免费起步 | ❌ | ⭐⭐⭐⭐ |
| **Claude Code** | 终端 Agent + 文件编辑 | API 按量 | ❌ | ⭐⭐⭐⭐ |
| **[Continue.dev](https://continue.dev)** | 开源 IDE 插件 | API 费用 | ✅ | ⭐⭐⭐ |
| **OpenCode** | CLI Agent + 技能系统 | API 费用 | ✅ 即将开源 | ⭐⭐⭐⭐ |
| **[Aider](https://aider.chat)** | Git 感知的终端 Agent | API 费用 | ✅ | ⭐⭐⭐⭐ |
...
| [⬅️ 上一阶段：生产化](./05-生产化与工程化.md) | [🏠 返回主指南](./README.md) | [📚 技术选型对比合集](./07-技术选型对比合集.md) |
|:---:|:---:|:---:|
