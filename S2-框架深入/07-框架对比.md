...
- [一、核心哲学差异](#一核心哲学差异)
  - [1.1 设计哲学总览](#11-设计哲学总览)
  - [1.2 设计哲学详解](#12-设计哲学详解)
  - [1.3 设计哲学公式](#13-设计哲学公式)
- [二、响应式原理深度对比](#二响应式原理深度对比)
  - [2.1-2.3 三大框架响应式](#21-vue-3--proxy-代理)
  - [2.4 渲染管线对比](#24-渲染管线深度对比)
  - [2.5 响应式总结](#25-响应式对比总结)
- [三、组件化方案对比](#三组件化方案对比)
  - [3.2 组件通信](#32-组件通信)
  - [3.3 信号化输入输出](#33-信号化输入输出2026-最新)
  - [3.4 控制流语法](#34-控制流语法对比)
  - [3.5 生命周期](#35-生命周期对比)
  - [3.6 延迟加载](#36-条件渲染与-suspense-延迟加载)
- [四、状态管理生态](#四状态管理生态)
  - [4.1 代码对比](#状态管理代码深度对比)
  - [4.2 性能基准](#状态管理性能基准)
- [五、路由方案](#五路由方案)
...
- [六、构建工具链](#六构建工具链)
  - [6.1 构建配置](#61-构建配置对比)
  - [6.2 构建产物](#62-构建产物对比)
  - [6.3 编译引擎性能](#63-编译引擎性能对比2026)
  - [6.4 Monorepo](#64-monorepo-方案对比)
  - [6.5 CI/CD](#65-cicd-与-devops-对比)
- [七、SSR/SSG 方案](#七ssrssg-方案)
  - [7.1 数据获取对比](#71-数据获取对比)
  - [7.2 部署平台适配](#72-部署平台适配)
- [八、生态全维度对比](#八生态全维度对比)
  - [8.1 UI 组件库](#81-ui-组件库生态)
  - [8.3 测试工具链](#83-测试工具链)
  - [8.7 AI 开发生态](#87-ai--llm-开发生态)
- [九、TypeScript 集成](#九typescript-集成)
- [十、安全与合规对比](#十安全与合规对比)
- [十一、性能优化策略](#十一性能优化策略)
- [十二、学习曲线与团队适配](#十二学习曲线与团队适配)
- [十三、版本演进路线图](#十三版本演进路线图)
- [十四、面试经典问答与深度辨析](#十四面试经典问答与深度辨析)
- [十五、技术选型决策树](#十五技术选型决策树)
- [十六、应用场景决策指南](#十六应用场景决策指南)
...
const useCounter = create<CounterState>()((set, get) => ({
  count: 0,
  double: computed(() => get().count * 2), // 需要手动维护
...
| **离开前** | `beforeRouteLeave` | `useBeforeUnload` | `canDeactivate` |
...
Angular 21 + esbuild 构建产物：
  dist/
    ├─ index.html          ~3KB
    ├─ main-xxx.js         ~200KB (gzip)  ← Angular 完整框架
    ├─ polyfills-xxx.js    ~10KB           ← Zone.js 等 polyfill
    ├─ styles-xxx.css      ~5KB
    └─ chunk-xxx.js                       ← 懒加载模块
    
注意：Angular 21 全面 Zoneless + 增量编译后，主包可降至 ~150KB
```
...
| **Rolldown** | Vite 6 (2026) | Rust | ~1.2s | <40ms | 下一代构建 |

```
冷构建性能（1000 组件工程）：
  Vite 6 / Rolldown:  ~1.2s  🥇
  Turbopack:          ~1.5s  🥈
  Angular CLI/esbuild: ~3s   🥉
  Webpack 5:          ~8s    ❌

HMR 性能（单文件修改）：
  Vite 6 / Rolldown:  ~30ms  🥇
  Turbopack:          ~40ms  🥈
...
| 对比项 | Nuxt 4 (Vue) | Next.js 15 (React) | Analog (Angular) |
|--------|-------------|-------------------|-------------------|
...
// app router 中顶层 await 自动 Suspense
...
Vue 3.6 (2025)           → Alien Signals（性能接近 Solid.js）
```
...
React 19 (2025)          → Actions/use() + React Compiler
```
...
// Zustand (React)
const useCounter = create<{ count: number; double: number; inc: () => void }>((set, get) => ({
  count: 0,
  double: computed: 0, // 需手动计算
...
| 能力 | Nuxt 4 (Vue) | Next.js 15 (React) | Analog (Angular) |
|------|-------------|-------------------|-------------------|
...
| 关联专题 | 文件 |
|---------|------|
| Vue 3 完整指南 | [01-Vue3.md](./01-Vue3.md) |
| React 19 完整指南 | [02-React19.md](./02-React19.md) |
| Angular 21 完整指南 | [03-Angular20.md](./03-Angular20.md) |
| 面试题库 | [01-前端面试题库.md](../S4-面试冲刺/01-前端面试题库.md) |
...
| [⬅️ 上一章：Angular 20](./03-Angular20.md) | [🏠 返回主指南](../README.md) | [🔝 返回顶部](#top) |
|:---:|:---:|:---:|
