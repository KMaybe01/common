...
| **Vue 3.5** | 2025 | 响应式系统优化、SSR 改进 | 更好的 DevTools 支持，内存优化 |
...
### Q3：Vue 3.4+ computed 缓存优化是什么？

**核心改进：** computed 在依赖值变化但计算结果不变时，不会通知下游 watcher

```javascript
const count = ref(0)
const isEven = computed(() => count.value % 2 === 0)

// count 从 0 → 2 时，isEven 虽然结果不变
// 但 Vue 3.4 之前仍会通知下游 watcher 重新计算
