...
| # | 模块 | 题数 |
|---|------|------|
| 🧠 | [一、Agent 基础篇](#一agent-基础篇) | 28 题 |
| 🔧 | [二、工具调用与协议篇](#二工具调用与协议篇) | 16 题 |
| 📐 | [三、大模型基础篇](#三大模型基础篇) | 84 题 |
| 🔗 | [四、LangChain 框架篇](#四langchain-框架篇) | 12 题 |
| 🚀 | [五、2025/2026 Agent 新趋势](#五20252026-agent-新趋势) | 新增 |

> 💡 **总计：** **共 140 道**面试题（含完整详解）
...
### Q11: 请比较 LangChain 和 LlamaIndex。它们的核心应用场景有何不同？

> 💡 **要点**：LangChain 强于 Chain/Agent 编排，LlamaIndex 强于数据索引与 RAG —— 核心哲学是"流程编排" vs "数据检索"

**LangChain** 的核心理念是 **Chain/Agent 编排流水线**，将 LLM 调用、工具使用、记忆管理组合为可复用的执行链路。**LlamaIndex** 则聚焦于 **数据索引与检索增强生成（RAG）**，提供丰富的 Data Connectors、索引结构和检索策略。

```mermaid
graph TB
    subgraph LangChain["LangChain 核心"]
        LC1["Chain 编排"] --> LC2["Agent 循环"]
        LC2 --> LC3["工具集成"]
        LC3 --> LC4["记忆管理"]
    end
    
    subgraph LlamaIndex["LlamaIndex 核心"]
        LI1["Data Ingestion"] --> LI2["索引构建"]
        LI2 --> LI3["检索策略"]
        LI3 --> LI4["Query Engine"]
    end
...
|------|-----------|------------|
| **核心抽象** | Chain / Agent / Tool | Index / Retriever / QueryEngine |
| **RAG 能力** | 基础检索 + 生成 | 深度索引：树/向量/关键词混合 |
| **Agent 能力** | 强（ReAct, OpenAI Function） | 较弱（Agent 为二等公民） |
| **Data Connectors** | 有限（Loader 方式） | 丰富（150+ 数据源原生支持） |
| **检索策略** | 简单 Top-K | 高级：递归检索、摘要索引、Router |
| **生态侧重** | 通用 LLM 应用编排 | 知识库/搜索/问答系统 |
| **学习曲线** | 中等（抽象层多） | 较低（API 聚焦数据管线） |

**选型建议：** 构建 **复杂多步骤 Agent**（如客服系统 + 多工具调用）→ **LangChain**；构建 **深度知识问答系统**（如企业知识库 + 多源检索）→ **LlamaIndex**。两者也可互补使用：LlamaIndex 作为 Retrieval 组件嵌入 LangChain 的 Agent 链条中。

---

### Q12: 你用过哪些 Agent 框架？选型是如何选的？评价指标是什么？

> 💡 **要点**：选型需权衡任务复杂度、团队技术栈、生态成熟度；评价聚焦任务成功率、单任务成本、端到端延迟

**主流 Agent 框架对比：**

| 框架 | 定位 | 语言 | 核心优势 | 适合场景 |
|------|------|------|---------|---------|
| **LangChain** | 通用编排框架 | Python/JS | 生态最大，组件全 | 通用 LLM 应用 |
| **LlamaIndex** | RAG 引擎 | Python | 检索能力最强 | 知识库搜索 |
| **Semantic Kernel** | 企业级 AI 编排 | C#/Python | .NET 原生，Azure 集成 | 微软生态企业 |
| **Dify** | 低代码平台 | Python | 可视化拖拽，快速原型 | 非技术人员/快速 MVP |
| **AutoGen** | Multi-Agent | Python | 多 Agent 对话模型 | 多角色协作系统 |

```mermaid
graph TD
    Task["任务类型分析"] --> Decision{"核心需求是？"}
    Decision -->|"复杂 Agent 编排"| LangChain["LangChain<br/>生态最大"]
    Decision -->|"深度 RAG 检索"| Llama["LlamaIndex<br/>检索最强"]
    Decision -->|"企业 .NET 生态"| SK["Semantic Kernel<br/>Azure 集成"]
    Decision -->|"低代码快速交付"| Dify["Dify<br/>可视化编排"]
    Decision -->|"多 Agent 协作"| AutoGen["AutoGen<br/>Multi-Agent"]
    
    LangChain --> Eval["评估指标体系"]
    Llama --> Eval
    SK --> Eval
    Dify --> Eval
    AutoGen --> Eval
...
**选型核心考量维度：**
- **任务复杂度**：单步 RAG → LlamaIndex；多步推理 + 工具 → LangChain；多角色协作 → AutoGen
- **团队技术栈**：Python 团队首选 LangChain/LlamaIndex；.NET 企业必选 Semantic Kernel
- **生态成熟度**：LangChain 社区最大，但 API 变动频繁；LlamaIndex 更稳定
- **交付效率**：Dify 适合快速验证概念；Semantic Kernel 适合合规严苛场景

**评价指标体系：**

| 指标 | 定义 | 衡量方式 |
|------|------|---------|
| **Task Success Rate** | 任务完成率 | 端到端正确完成比例 |
| **Cost Per Task** | 单任务 Token 消耗 | LLM 调用费 + Embedding 费 |
| **Latency (P50/P99)** | 端到端响应延迟 | 用户感知等待时间 |
| **Robustness** | 异常恢复能力 | 重试率、Fallback 触发率 |
| **Tool Call Accuracy** | 工具调用准确率 | 参数正确匹配比例 |
| **Context Utilization** | 长上下文利用率 | 有效信息命中率 |

**实践经验：** 建议先用 **Dify 快速验证** MVP，B 端复杂场景迁移至 **LangChain + LlamaIndex 组合**（LlamaIndex 负责检索、LangChain 负责编排），大规模部署时接入 **LangSmith** 进行全链路追踪和 Cost Analysis，持续优化 Task Success Rate 和 Cost Per Task。

---

## 🚀 五、2025/2026 Agent 新趋势

### Q1: 2025/2026 年 Agent 领域有哪些重要进展？

> 💡 **要点**：2025 年 Agent 领域迎来重大突破，MCP 协议标准化、Model Context Protocol 普及、Agent 编排框架升级、前端 AI 原生应用成为趋势。

**2025/2026 Agent 核心趋势：**

```mermaid
graph TD
    A["2025/2026 Agent 趋势"] --> B["MCP 协议标准化"]
    A --> C["前端 AI 原生"]
    A --> D["Agent 编排升级"]
    A --> E["本地化部署"]
    
    B --> B1["Anthropic MCP 1.0"]
    B --> B2["跨模型工具调用"]
    B --> B3["前端集成 MCP Client"]
    
    C --> C1["React/Vue AI 组件"]
    C --> C2["流式 UI 渲染"]
    C --> C3["AI 驱动状态管理"]
...
| 层级 | 技术 | 2025/2026 更新 |
|------|------|---------------|
| **协议层** | MCP、OpenAI Function Calling | MCP 成为行业标准 |
| **框架层** | LangChain、LlamaIndex、Dify | 支持 MCP、流式输出优化 |
| **模型层** | GPT-4o、Claude 3.5、Gemini 2.0 | 多模态、更长上下文 |
| **部署层** | Vercel AI SDK、Cloudflare AI | 边缘推理、本地化 |
| **前端层** | AI SDK、Vercel AI SDK | 流式 UI、React Server Components |

### Q2: 前端如何构建 AI 原生应用？

> 💡 **要点**：2025 年前端 AI 原生应用核心模式：流式 UI 渲染、AI 驱动状态管理、智能组件、本地推理，Vercel AI SDK 成为标准方案。
...
async function ChatMessage({ input }: { input: string }) {
  const result = await streamUI({
    model: openai('gpt-4o'),
    prompt: input,
    text: ({ content }) => <Markdown content={content} />,
    tools: {
      search: {
...
> 💡 **要点**：2025 年 WebLLM、Transformers.js 等库使浏览器内 AI 推理成为可能，隐私优先、零服务器成本、离线可用成为前端 AI 新趋势。
...
async function initLocalAI() {
  ...
  const chunks = await engine.chat.completions.create({
    messages: [{ role: 'user', content: '解释 React Server Components' }],
    stream: true,
  });
...
const result = await classifier('这段代码性能很好');
console.log(result); // [{ label: 'positive', score: 0.95 }]
...
| 隐私 | 数据上传 | 本地处理 |
| 成本 | 按 Token 计费 | 零服务器成本 |
| 离线 | 不可用 | 可用 |
| 模型大小 | 无限制 | < 2GB（量化后） |

### Q4: 2026 年 Agent 工程师面试重点？

> 💡 **要点**：2026 年 Agent 面试重点关注：MCP 协议实践、前端 AI 原生架构、本地推理优化、Agent 评估体系、多模态交互。

**2026 面试核心考点：**

| 考点 | 重要程度 | 准备建议 |
|------|---------|---------|
| **MCP 协议** | ⭐⭐⭐⭐⭐ | 实践 MCP Client/Server 开发 |
| **流式 UI** | ⭐⭐⭐⭐⭐ | 掌握 Vercel AI SDK |
| **Agent 架构** | ⭐⭐⭐⭐ | 理解 ReAct、Plan-Execute |
| **本地推理** | ⭐⭐⭐⭐ | 了解 WebLLM、Transformers.js |
| **评估体系** | ⭐⭐⭐ | 熟悉 RAGAS、DeepEval |
| **多模态** | ⭐⭐⭐ | 了解图像/音频处理 |

**2026 Agent 最佳实践：**
1. **协议优先**：使用 MCP 标准化工具调用
2. **流式优先**：所有 AI 响应使用流式输出
3. **本地优先**：敏感数据本地处理
4. **评估驱动**：建立 Agent 评估体系
5. **渐进增强**：云端 + 本地混合架构
6. **隐私保护**：用户数据最小化收集
