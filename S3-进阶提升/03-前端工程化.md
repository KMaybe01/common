...
| **CSS 处理** | Lightning CSS（Vite 6+） | CSS 压缩、降级、自动代码分割（Vite 8） |
...
> 💡 **要点**：Vite 8 正式使用 Rust 编写的 Rolldown 作为默认生产构建引擎，兼容 Rollup API，构建速度提升 5-30x，内存降低 60%+。
...
| **内存使用** | 相比 Vite 6 降低约 30% | 相比 Vite 7 再降低约 20% |
...
### 1️⃣0️⃣ **Vite 的 manualChunks 与代码分割**

> 💡 **要点**：`manualChunks` 是 `build.rollupOptions.output` 下的配置项，用于自定义代码分割策略。Vite 8 中该配置兼容 Rolldown，与 Webpack 的 `splitChunks` 实现机制完全不同。

**`manualChunks` 在构建流程中的位置：**
...
**`manualChunks` vs Webpack `splitChunks`：**
...
| **底层机制** | Rollup/Rolldown chunk 分配 | Webpack seal 阶段 |
...
| 优化项 | 方式 | 效果 |
|--------|------|------|
| **代码分割** | `manualChunks` 拆分 vendor/UI 库/业务代码 | 并行加载，缓存分离 |
...
3. **标准化项目模板**：使用脚手架（如 `create-vite`、Yeoman）生成统一项目结构
...
常见脚手架：Vite CLI、create-react-app、@angular/cli、vue-cli、Yeoman、create-turbo。
