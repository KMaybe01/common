...
> 💡 **总结**：Vite 通过架构创新（ESM 开发 + esbuild 预构建 + Rollup 生产）解决了 Webpack 冷启动慢和 HMR 随项目增大衰减的核心痛点，已成为新项目的事实标准。存量 Webpack 项目不建议大规模重构，可考虑渐进迁移或使用 Rspack 作为替代。
...
**2025 年状态**：Vite 6 默认使用 Rolldown 进行生产构建，生态逐步完善。

### 3️⃣ Vite 6 新特性

> 💡 **要点**：Vite 6（2024 年底发布）引入 Rolldown 生产构建、改进的 HMR、原生 ESM 优化、更好的 SSR 支持，2025 年成为主流版本。
...
export default defineConfig({
  ...
  optimizeDeps: {
    ...
    holdUntilCutoff: true, // 新：延迟预构建直到构建完成
  },
