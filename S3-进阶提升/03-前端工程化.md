...
> 🎯 **面试星级**：★★★★☆ | **建议用时**：2 天
> 前端工程化核心知识全面梳理，涵盖模块化、Webpack/Vite、Babel、Monorepo、微前端、CI/CD 等核心考点
...
### 1️⃣1️⃣ 如何提⾼**webpack**的打包速度**?**

> 💡 **要点**：多进程编译（thread-loader）、外部扩展（externals）、Webpack 5 持久化缓存、缓存策略、缩小文件搜索范围（include）。

- thread-loader: 利⽤进程并⾏编译loader，替代已废弃的 HappyPack，配置在 babel-loader 前
...
- 利⽤缓存: babel-loader.cacheDirectory 可以利⽤缓存提⾼rebuild效率
...
### 1️⃣2️⃣ 如何提⾼**webpack**的构建速度？

> 💡 **要点**：SplitChunksPlugin 提取公共代码、externals 分离第三方库、Webpack 5 持久化缓存、thread-loader 多线程加速。

1. 多⼊⼝情况下，使⽤ SplitChunksPlugin 来提取公共代码（替代已废弃的 CommonsChunkPlugin）
2. 通过 externals 配置来提取常⽤库
3. 利⽤ Webpack 5 持久化缓存（`cache: { type: 'filesystem' }`），替代 DllPlugin
4. 使⽤ thread-loader 实现多线程加速编译（替代已废弃的 HappyPack）
5. 使⽤ terser-webpack-plugin（Webpack 5 内置，production 模式默认启用）并开启 parallel 提升压缩速度
6. 使⽤ Tree-shaking 和 Scope Hoisting 来剔除多余代码

### 1️⃣3️⃣ 怎么配置单⻚应⽤？怎么配置多⻚应⽤？
...
> 💡 **要点**：基于 ESM 的开发服务器，esbuild 预构建依赖实现秒级启动，生产构建使用 Rollup。热更新仅编译变更模块，大型项目中大幅优于 Webpack。2024-2026 年已成为前端新项目主流选择。
...
> 💡 **要点**：Vercel 开发的 Rust 增量打包器，函数级缓存仅重新计算变更部分。Next.js 15+ 默认打包器，已支持生产构建，生态逐步成熟。
...
### 4️⃣ SWC (Speedy Web Compiler)

> 💡 **要点**：Rust 编写的 JS/TS 编译器，比 Babel 快 10-15 倍。被 Next.js、Deno、Parcel 等广泛采用，支持转译、压缩和打包。
...
> 💡 **要点**：字节跳动的 Rust 打包器，高度兼容 Webpack API 和生态，可直接迁移 Webpack 配置，性能提升 5-10 倍。
...
> 💡 **要点**：Zig 编写的一体化 JS 运行时，集运行、打包、转译、包管理、测试于一身。基于 JavaScriptCore 引擎，启动比 Node.js 快 4 倍。
...
> 💡 **要点**：安全优先的 JS/TS 运行时，Rust + V8 引擎，默认无文件/网络/环境权限，原生支持 TypeScript，支持 URL 导入和 npm: 前缀。
...
### 2️⃣ Module Federation (Webpack 5)
...
> 💡 **要点**：基于 single-spa 的微前端框架，提供完整的应用生命周期（bootstrap/mount/unmount）和 Proxy 沙箱 JS 隔离机制。
...
> 💡 **要点**：Husky 管理 Git hooks，lint-staged 仅检查暂存文件提升效率，commitlint 规范提交信息格式（type(scope): message）。
...
> 💡 **要点**：基于 Vite 的测试框架，兼容 Jest API。原生支持 ESM 和 TypeScript，支持测试 HMR 热更新，esbuild 转译速度远超 Jest。
...
> 💡 **要点**：基于事件的 CI/CD 平台。Workflow 由触发事件（push/PR）、Job 和 Step 组成。可自动执行 lint → test → build → deploy 流程。
...
> 💡 **要点**：GitLab CI 使用 `.gitlab-ci.yml` 定义 Pipeline，Runner 执行 Job。与 GitLab 仓库深度集成，支持自托管 Runner、Docker/K8s 执行器、多环境部署、Review Apps 自动预览。
...
**GitLab Runner 配置（Docker 执行器）：**
```yaml
# config.toml (Runner 配置)
concurrent = 4
...
| **Runner** | GitHub 托管 / 自托管 | 自托管为主（GitLab SaaS Runner 免费额度） |
| **Pipeline 结构** | Workflow → Job → Step | Pipeline → Stage → Job |
| **缓存机制** | actions/cache（需配置） | 内置 cache / artifacts |
| **Review Apps** | 需要第三方 action | 原生支持（environment: on_stop） |
| **容器注册表** | GitHub Container Registry | GitLab Container Registry（内置） |
| **Pages 部署** | GitHub Pages（需 action） | GitLab Pages（原生 CI 部署） |
| **自托管成本** | 免费 | 免费（不限并发 Runner） |
| **K8s 集成** | 通过 action 集成 | 原生 Kubernetes executor |
| **CI 分钟数** | 2000 分钟/月（免费） | 400 分钟/月（免费） |

**GitLab CI 独有特性：**
- **父子 Pipeline**：`trigger` 关键字触发子 Pipeline，适合 Monorepo
- **Merge Trains**：合并队列，自动合并多个 MR 的 Pipeline 结果
- **CI/CD 变量继承**：环境级别变量覆盖项目级别变量
- **内置 SAST/DAST**：集成安全扫描（需 Ultimate 版）
- **`.gitlab-ci.yml` includes**：复用模板文件

```yaml
# 父子 Pipeline 示例（Monorepo 场景）
# 父 Pipeline
stages:
  - trigger
...
trigger-packages:
  stage: trigger
  trigger:
    include: packages/*/.gitlab-ci.yml
    strategy: depend
```

**GitLab CI 最佳实践：**
1. 使用 `rules` 替代 `only/except`（`only/except` 已弃用）
2. 合理设置 `cache` 和 `artifacts` 过期时间，避免存储溢出
3. 自托管 Runner 添加 `tags` 标签，精确控制 Job 分配
4. 敏感信息存 CI/CD Variables，不要硬编码
5. Monorepo 使用 `changes` 规则只构建变更项目

### 3️⃣ Docker 前端部署

> 💡 **要点**：多阶段构建分离构建环境和运行环境，减小镜像体积。Nginx 配置 `try_files` 处理 SPA 路由，静态资源设置强缓存。
