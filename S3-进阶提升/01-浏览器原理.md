...
- [🌍 七、浏览器同源策略](#-七浏览器同源策略)
- [🔄 八、浏览器事件机制](#-八浏览器事件机制)
- [🗑️ 九、浏览器垃圾回收机制](#-九浏览器垃圾回收机制)
- [🛡️ 十、现代浏览器安全新特性](#-十现代浏览器安全新特性)
- [🚀 十一、现代浏览器特性](#-十一现代浏览器特性)
- [📊 十二、Web性能API](#-十二web性能api)
- [⚡ 十三、浏览器渲染进阶](#-十三浏览器渲染进阶)
- [💿 十四、Web存储进阶](#-十四web存储进阶)
- [🏛️ 十五、浏览器架构演进](#-十五浏览器架构演进)
- [🔮 十六、2025/2026 浏览器新特性](#-十六20252026-浏览器新特性)
...
sequenceDiagram
    ...
    User->>Target: 登录并保持session
    User->>Attack: 诱导点击恶意链接
    Attack->>Target: 自动发起跨站请求(携带cookie)
...
> 💡 **要点**：渲染五阶段：JS → Style → Layout → Paint → Composite。transform/opacity 仅触发 Composite 性能最好，width/height 触发完整流水线。
...
timeline
    ...
    2016-2020 : Chrome 多进程全面
              ...
              : 浏览器标准高度统一
    2021-2026 : 所有主流浏览器基于 Chromium/WebKit
              : 隐私沙盒 + AI 原生支持
              : WebGPU/WebNN 标准落地
```
...
**2025 年状态**：Chrome 120+ 稳定支持，Firefox 实验性支持，Safari 考虑中。Next.js 15+ 和 Astro 5 已内置集成。

### 2️⃣ View Transitions API 2.0

> 💡 **要点**：从 SPA 扩展到 MPA（多页应用），支持跨页面导航动画、共享元素过渡、自定义动画时间线，2025 年成为跨页面动效标准方案。

View Transitions API 在 2025 年迎来重大更新：
...
> 💡 **要点**：Chrome 125+ 默认禁用第三方 Cookie，替代方案：CHIPS（Partitioned Cookie）、Privacy Sandbox API（Attribution Reporting、Topics）、First-Party Context。

2025 年 Chrome 正式淘汰第三方 Cookie，前端需适配：
...
| [🏠 返回主指南](../README.md) | [➡️ 下一章：性能优化](./02-性能优化.md) |
|:---:|:---:|
