...
- [一、HTTP 发展史](#一http-发展史)
- [二、HTTPS 与 TLS](#二https-与-tls)
- [三、TCP 与 UDP](#三tcp-与-udp)
- [四、DNS 解析](#四dns-解析)
- [五、浏览器缓存机制](#五浏览器缓存机制)
- [六、CDN 原理与实践](#六cdn-原理与实践)
- [七、CORS 跨域](#七cors-跨域)
- [八、网络安全（前端视角）](#八网络安全前端视角)
- [九、Cookie / Session / Token](#九cookie--session--token)
- [十、HTTP 状态码](#十http-状态码)
- [十一、WebSocket 与 SSE](#十一websocket-与-sse)
- [十二、HTTP/3 与 WebTransport](#十二http3-与-webtransport)
- [十三、面试高频问答](#十三面试高频问答)
...
- **Q**：HTTPS 对性能有多大影响？
- **A**：现代机器上对称加密的额外延迟约 1-3%，主要在握手阶段。HTTP/2 多路复用可减少连接数，配合会话复用（Session Resumption）降低握手开销。
...
| 关联专题 | 文件 |
|---------|------|
| 浏览器原理（安全/缓存详情） | [07-浏览器原理.md](./07-浏览器原理.md) |
| 性能优化（网络层面） | [08-性能优化.md](./08-性能优化.md) |
| 面试题库（网络专题） | [11-前端面试题库.md](../S4-面试冲刺/11-前端面试题库.md) |
| 工程化（CI/CD/构建） | [09-前端工程化.md](./09-前端工程化.md) |
